...
def _find_column_index(header_values: list, requested_name: str | None, candidates: list[str]) -> int | None:
    ...
    def exact_match(name: str) -> int | None:
        """Try exact matching first (case-insensitive, whitespace-insensitive)."""
    ...
    def fuzzy_match(name: str) -> int | None:
        """Fall back to partial/fuzzy matching."""
        n = _normalize_header(name)
        if not n:
            return None
        for i, v in indexed:
            v_norm = _normalize_header(v)
            if n in v_norm or v_norm in n:
                return i
        return None
    ...
    # Try exact matches first on candidates
    for c in candidates:
        found = exact_match(c)
        if found:
            return found

    # Then try fuzzy matches on candidates
    for c in candidates:
        found = fuzzy_match(c)
        if found:
            return found
...
def _parse_args():
    ...
    parser.add_argument("--action-timeout-ms", type=int, default=120000)  # Longer timeout for action button
    ...
    with sync_playwright() as p:
        ...
        for row_index in range(header_row + 1, int(ws.max_row or 0) + 1):
            ...
            try:
                ...
                if action_locator:
                    try:
                        action_locator.click(timeout=max(5000, int(args.action_timeout_ms)))
                    except Exception:
                        pass  # Continue even if button click fails

                page.wait_for_timeout(max(0, int(args.wait_ms)))
                
                # Wait for visible content - retry many times if empty
                actual_output = ""
                tries = max(1, int(args.retries) * 2)  # Double the retry attempts
                ...
                if expected_sinhala:
                    ...
                    status = "COLLECTED" if actual_output else "NO_OUTPUT"
